    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: '32px' }}>
        <div>
          ...
          <p style={{ fontSize: '0.85rem', color: 'var(--muted)', marginTop: '6px' }}>Your financial snapshot</p>
        </div>
        <div style={{ display: 'flex', gap: '4px', background: 'var(--surface)', padding: '4px', borderRadius: '10px', border: '1px solid var(--border)' }}>
          {RANGES.map((r) => (
            <button
              key={r}
              onClick={() => setRange(r)}
              style={{ padding: '7px 14px', borderRadius: '7px', border: 'none', fontSize: '0.8rem', fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', background: range === r ? 'var(--accent)' : 'transparent', color: range === r ? '#0a0a0f' : 'var(--muted)', transition: 'all 0.15s' }}
            >
              {r}
            </button>
          ))}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '24px' }}>
        {[
          ...
          <div key={card.label} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '16px', padding: '24px', position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '2px', background: card.color }} />
            <p style={{ fontSize: '0.72rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '10px' }}>{card.label}</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '16px', marginBottom: '24px' }}>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '16px', padding: '28px' }}>
          ...
          {barData.length === 0 ? (
            <div style={{ height: '260px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted2)', fontSize: '0.875rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '16px', padding: '28px' }}>
          <p style={{ fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.09em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '20px' }}>Spending by Category</p>
          {pieData.length === 0 ? (
            <div style={{ height: '260px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted2)', fontSize: '0.875rem', textAlign: 'center' }}>
              No expenses yet
            </div>
          ) : (
            <ResponsiveContainer width="100%" height={260}>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '16px', padding: '28px' }}>
        <p style={{ fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.09em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '20px' }}>Top Spending Categories</p>
        {topSpending.length === 0 ? (
          <p style={{ color: 'var(--muted2)', fontSize: '0.875rem', textAlign: 'center', padding: '20px' }}>No expenses yet</p>
        ) : (
          <div>
            {topSpending.map((cat, i) => {
              const pct = totalExpense > 0 ? (cat.value / totalExpense) * 100 : 0
              return (
                <div key={cat.name} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '12px 0', borderBottom: i < topSpending.length - 1 ? '1px solid var(--border)' : 'none' }}>
                  <span style={{ fontSize: '0.75rem', color: 'var(--muted)', width: '20px', fontFamily: 'monospace' }}>#{i + 1}</span>
                  ...
                  <span style={{ fontSize: '0.75rem', color: 'var(--muted)', minWidth: '40px', textAlign: 'right' }}>{pct.toFixed(0)}%</span>
                </div>
              )
            })}
          </div>
